...
__global__ void self_ref_forward_kernel(
    ...
    const float_accessor input_to_y,
    float_accessor Wy,
    float_accessor Wq,
    float_accessor Wk,
    float_accessor wb,
    ...
    float* shared_input_to_y = shared_x + E_block*T;
    // float* shared_x = shared_wb + E_block * 4;

    float* shared_results = shared_input_to_y + E_block*T;
         ...
         i += blockDim.x)
    {
        ...
        if (d < E) {
            ...
            shared_input_to_y[i] = input_to_y[n][h][t][d];
        }
    }
    __syncthreads();
    if (n >= N) {
    ...
    for (int t=0; t<t_end; t++) {  // main loop over time in the segment
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            kv_idx = threadIdx.x + sub * blockDim.x;
            if (e < E) {
                y_main = shared_Wy[kv_idx] * shared_input_to_y[t*E_block + e];
...
// Forward
void self_ref_forward_v3(
    const torch::Tensor x,
    const torch::Tensor input_to_y,
    torch::Tensor W_y,
    torch::Tensor W_q,
    torch::Tensor W_k,
    torch::Tensor w_b,
...
//    const at::cuda::OptionalCUDAGuard device_guard(device_of(queries));
    ...
    const int shared_mem_per_time = E_block*2;
    ...
    for (int l_offset=0; l_offset < L; l_offset += T) {
        self_ref_forward_kernel
            ...
            input_to_y.packed_accessor32<float, 4, torch::RestrictPtrTraits>(),
...
__global__ void self_ref_backward_kernel(
    ...
    float_accessor grad_input_to_y,
    ...
    float* shared_grad_input_to_y = shared_grad_x + M;

    float* shared_gradout = shared_grad_input_to_y + M;
    float* shared_x = shared_gradout + M*T;
    float* shared_input_to_y = shared_x + E_block*T;
    float* shared_q = shared_input_to_y + E_block*T;
    ...
    for (int i = threadIdx.x; i < (t_end*E_block); i += blockDim.x)
    {
        ...
        if (d < E) {
            ...
            shared_input_to_y[i] = input_to_y[n][h][t_bw][d];
        }
    }
    ...
    for (int t=0; t<t_end; t++) {
        ...
        for (int sub=0; sub<subblocks_per_seq; sub++) {
            ...
            if (e < E) {
                shared_grad_Wy[kv_idx] +=
                  shared_gradout[m_abs] * shared_input_to_y[e_abs];
            }
        }
        __syncthreads();
        ...
        if (threadIdx.x < M) {
            ...
            shared_grad_x[m] = 0.f;
            atomicAdd(
                &grad_input_to_y[n][h][l_b][m],
                shared_grad_input_to_y[m]
            );
            shared_grad_input_to_y[m] = 0.f;
            shared_grad_q[m] = 0.f;
            shared_grad_k[m] = 0.f;
...
//    const at::cuda::OptionalCUDAGuard device_guard(device_of(grad_queries));
    ...
    int shared_mem_const = (3 * E_block * M + 4 * M) * 2 + 2 * (3 * M + 4) + 1 + M;

    // 5M for value, rnn_out, rnn_delayed, grad_out, tmp_grad.
    // E for key.
    int shared_mem_per_time = 2 * (3*M + 4) + E_block*2;
    ...
    for (int l_offset=0; l_offset < L; l_offset += T) {
        self_ref_backward_kernel
            ...
            grad_input_to_y.packed_accessor32<float, 4, torch::RestrictPtrTraits>(),
